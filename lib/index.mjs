...
async function psg_input_wait(psg_count){
    ...
    console.log(`Sleeping for ${sleep_time * 0.0025} seconds for passenger details input.`);
    await normal_sleep(sleep_time*2.5);
...
async function pay_with_upi(book_params,params){
    ...
    if (Object.prototype.hasOwnProperty.call(book_params.payment_webdata, "paramList")){
        ...
        // Check if we got PayTM page directly or need to submit a form
        if (response.body && typeof response.body === 'string' && response.body.includes('var pushAppData="')) {
            // We got the PayTM page directly
            book_params.paytmgw.landing = response.body;
        } else if (response.body && typeof response.body === 'string' && response.body.includes('<form action="')) {
            // We got a redirect form, submit it
            ...
            console.log('Payment redirect response does not contain expected structure');
            console.log('Response body type:', typeof response.body);
            console.log('Response body preview:', response.body ? response.body.substring(0, 500) : 'null');
            throw new Error('Payment redirect failed - invalid response format');
        }
...
async function paytmgw_core(book_params={},params={}){
    // Check if landing page data exists and contains expected structure
    if (!book_params.paytmgw.landing || typeof book_params.paytmgw.landing !== 'string' || !book_params.paytmgw.landing.includes('var pushAppData="')) {
        console.log('Landing page data missing or invalid');
        console.log('Landing data:', book_params.paytmgw.landing);
        throw new Error('Payment gateway landing page data is missing or invalid. Please check the payment redirect response.');
    }
...
class IRCTC{
    ...
    async book(params={}){
        try {
            ...
            // Keep browser open until explicitly closed
            console.log('Booking process completed - browser session maintained');
        }
    }

    async closeBrowser() {
        await this.browse.closeBrowser();
    }
    ...
    async last_transaction(){
        try {
            ...
            console.log('Transaction check completed - browser session maintained');
        }
    }
    ...
    async pnr_status(pnr=""){
        try {
            ...
            console.log('PNR status check completed - browser session maintained');
        }
    }
}
